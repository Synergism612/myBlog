...
export default defineComponent({
  setup() {
    ...
    const save = (): void => {
      api.saveWriteArticle(viewData.articleForm);
    };
    ...
    const beforeAvatarUpload: UploadProps["beforeUpload"] = (rawFile) => {
      if (rawFile.type !== "image/jpeg") {
        Message.errorMessage("Avatar picture must be JPG format!");
        return false;
      } else if (rawFile.size / 1024 / 1024 > 2) {
        Message.errorMessage("Avatar picture size can not exceed 2MB!");
        return false;
      }
      return true;
    };
